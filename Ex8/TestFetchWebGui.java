...
public class TestFetchWebGui {
    ...
    private static void goodFetch() {
        ...
        List<DownloadWorker> workers = new ArrayList<>(urls.length);
        for (String url : urls)
            workers.add(new DownloadWorker(textArea, url, counter));
        fetchButton.addActionListener((e) -> workers.forEach((w) -> w.execute()));

        // (2) Add a progress bar
        JProgressBar progressBar = new JProgressBar(0, urls.length);
    ...
    static class DownloadWorker extends SwingWorker<String,String> {
        ...
        public DownloadWorker(TextArea textArea, String url, AtomicInteger counter) {
        ...
        public String doInBackground() {
            return isCancelled() ? "" : String.format("%-40s%7d%n", url, getPage(url, 200).length());
        }

        // Called in the event thread when doInBackground() has terminated,
        // whether by completing or by being cancelled.
        public void done() {
            try {
                textArea.append(get());
                setProgress(counter.incrementAndGet()); // (2)
            }
